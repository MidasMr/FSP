...
3. **Доступ к приложению**: Серивс доступен по данному адресу `http://localhost:8008`.
...
3. **Отредактируйте env файл в соотвествии с созданной базой**
...
Проект выполнен с использованием **Vertical Slice Architecture**. Этот подход подразумевает разделение приложения на независимые вертикальные срезы. Такой подход делает приложение более поддерживаемым и расширяемым.
...
Стоит отметить, что при проектировании приложения было решено сделать соединения межжду городами уникальными. Реализация уникальных соединений между городами в проекте позволила гарантировать,
что для любых двух городов существует только одно соединение, которое рассматривается как в прямом, так и в обратном направлении. Это обеспечило не только целостность данных, но и упрощение работы
с ними, поскольку не требуется обрабатывать дублирующие маршруты или учитывать порядок городов при поиске.
...
Проверить выполнение запроса можно, несколькими способами:

### Postgres admim

панель администрирования PostgreSQL, через которую можно выполнить запрос в нужной базе данных.

### Командная строка

Для проверки работы sql запроса необходимо выполнить следующие действия:

1. **Собрать и запустить контейнеры, как уже было описано ранее**
2. **Импортировать данные или наполнить своими**
3. **В командной строке прописать**:

    ```bash
    docker exec -it fsp-db bash
    ```

    В открывшемся терминале контейнера выполнить:
...
4. **Далее скопировать скрипт из файла и вставить его в компандную строку**
